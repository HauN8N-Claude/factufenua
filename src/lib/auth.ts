    ...
    user: {
      create: {
        after: async (user, _req) => {
          // MVP : Resend non configuré — ne doit pas bloquer l'inscription.
          try {
            await setupResendCustomer(user);
          } catch (err) {
            logger.warn("setupResendCustomer skipped (MVP)", { err });
          }
    ...
    organization({
      ...
      organizationHooks: {
        async afterCreateOrganization(data: {
          ...
          // MVP : Stripe non activé (premium à 500 users). Ne doit jamais
          // bloquer la création d'organisation / l'inscription.
          try {
            ...
            logger.warn("Stripe customer creation skipped (MVP)", { err });
          }
        },
      },
